...
static const char sharkTrustRequestHeader[]={
   "GET /device/ HTTP/1.0\n"
   "X-Key: " /* Header key, the header value is added by C code below */
};
...
int openSharkTrustCon(SSL_CTX** ctx, SSL** ssl)
{
   ...
   *ssl = SSL_new(*ctx);
   ...
   getZoneKey(zoneKey); /* Auto generated, see getzkey.ch */
   SSL_write(*ssl,zoneKey, 64);
   SSL_write(*ssl,"\nX-Command: ", 12);
   return ssock;
}
...
int getSharkTrustResponse(SSL *ssl, uint8_t* buf, int blen)
{
   ...
   switch(buf[2])
   {
      case RSP_FORBIDDEN: /* Invalid SharkTrust Zone Key */
